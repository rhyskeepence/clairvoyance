...
object FromSource {
  ...
  @tailrec
  def readToEndOfMethod(
      content: Seq[String],
      lineNumber: Int,
      indentLevel: Int = 0,
      res: List[(Int, String)] = List()
  ): List[(Int, String)] = {
    if (content.size < lineNumber || lineNumber < 1) {
      res.reverse

    } else if (content(lineNumber)
                 .trim()
                 .contains("{") && content(lineNumber).trim().contains("}")) {
      readToEndOfMethod(content, lineNumber + 1, indentLevel, addLine(lineNumber, content, res))

    } else if (content(lineNumber)
                 .trim()
                 .contains("{") || content(lineNumber).trim().endsWith("=")) {
