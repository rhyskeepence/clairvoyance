...
object Reflection {
  ...
  /**
    ...
    */
  def tryToCreateObject[T <: AnyRef](
      className: String,
      ...
      case Right(c: Class[_]) =>
        try {
          val constructors = c.getDeclaredConstructors.toList
            ...
            .sortBy(_.getParameterTypes.size)
          if (constructors.isEmpty)
            Left(new Exception("Can't find a constructor for class " + c.getName))
          else {
            val results =
              constructors.map(constructor => createInstanceForConstructor(c, constructor, None))
            results.find(_.isRight) match {
              case Some(r) => r
              case None =>
                val exception = results
                  .collect {
                    case Left(e) => e
                  }
                  .iterator
                  .toSeq
                  .headOption
                  .getOrElse(new Exception("no cause"))
                Left(
                  new Exception(
                    "Could not instantiate class " + c.getName + ": " + exception.getMessage,
                    exception
                  )
                )
            }
          ...
          case e: Throwable =>
            Left(new Exception("Could not instantiate class " + className + ": " + e.getMessage, e))
        }
    }
  }
  ...
  /**
    ...
    */
  private def loadClassEither[T <: AnyRef](
      className: String,
      loader: ClassLoader
  ): Either[Throwable, Class[T]] =
    tryEither(loadClassOf(className, loader).asInstanceOf[Class[T]]) {
      case e =>
  ...
  private def createInstanceForConstructor[T <: AnyRef: ClassTag](
    ...
    if (constructor.getParameterTypes.isEmpty)
      ...
      constructorParameter.map(constructor.newInstance(_).asInstanceOf[T]).toRight {
        new Exception(
          "can't create an instance of " + c + " for a constructor with parameter " + constructorParameter
        )
      }
  ...
  /**
    ...
    */
  private def createInstanceOfEither[T <: AnyRef](
      c: Option[Class[T]]
  ...
  private def createInstanceFor[T <: AnyRef](klass: Class[T])(implicit m: ClassTag[T]): T = {
    ...
    if (!m.runtimeClass.isInstance(instance))
      sys.error(instance + " is not an instance of " + m.runtimeClass.getName)
  ...
  /**
    ...
    */
  private def getOuterClassName(c: Class[_]): String =
