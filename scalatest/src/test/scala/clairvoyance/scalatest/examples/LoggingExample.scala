...
import org.scalatest.{Matchers, Spec, Suite}
...
trait DoomsdayContext extends ClairvoyantContext with Matchers{ this: Suite =>
  ...
  def givenTheDateIs(ddMMyyyy: String): Unit = clock.setDateTo(toDate(ddMMyyyy))
  private def toDate(ddMMyyyy: String) = new SimpleDateFormat("dd/MM/yyyy").parse(ddMMyyyy)

  def whenTheCoordinatorRuns(): Unit = new MasterCoordinator(theDoomsdayDevice, clock).runIt()
