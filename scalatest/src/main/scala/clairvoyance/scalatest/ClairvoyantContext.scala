...
trait ClairvoyantContext extends SuiteMixin with InterestingGivens with CapturedInputsAndOutputs {
  this: Suite =>
  ...
  abstract override def run(testName: Option[String], args: Args): Status = {
    if (testName.isEmpty) { // so we don't produce a report for every test when OneInstancePerTest
      ...
      val status: Status =
        super.run(testName, args.copy(reporter = new ForwardingReporter(args.reporter, reporter)))
      reporter.done()
      status
  ...
  abstract override protected def runTest(testName: String, args: Args): Status = {
    tagNames += (
      (
        (suiteName, testName),
        tags.withDefaultValue(Set.empty)(testName).map(normaliseTagName)
      )
    )
  ...
  private def normaliseTagName: (String) => String =
    tagName =>
      if (tagName.startsWith(INTERFACE_PREFIX)) tagName.substring(INTERFACE_PREFIX.length)
      else tagName
